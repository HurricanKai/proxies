...
use crate::server::consts::SOCKS5_ADDR_TYPE_IPV4;
use crate::server::SocksError;
use anyhow::Context;
use log::debug;
use std::fmt;
use std::io;
...
impl TargetAddr {
    pub async fn resolve_dns(self, should_ipv4: bool) -> anyhow::Result<TargetAddr> {
        match self {
            ...
            TargetAddr::Domain(domain, port) => {
                ...
                let socket_addr = lookup_host((&domain[..], port))
                    ...
                    .filter(|x| x.is_ipv4() == should_ipv4)
                    .next()
